use indexing::store::{
    Aggregation, DgraphStore, ElasticsearchStore, Filter, FilterOperator, GraphStore,
    IndexedObject, SearchQuery, SearchStore, TraversalAggregation,
};
...
async fn create_test_dgraph_store() -> Option<DgraphStore> {
    let endpoint =
        std::env::var("DGRAPH_URL").unwrap_or_else(|_| "http://localhost:9080".to_string());
...
async fn test_elasticsearch_count_objects() {
    ...
    properties2.insert(
        "name".to_string(),
        PropertyValue::String("Test 2".to_string()),
    );
    properties2.insert("value".to_string(), PropertyValue::Integer(20));

    // Index objects
    store
        .index_object(object_type, "id1", &properties1)
        .await
        .unwrap();
    store
        .index_object(object_type, "id2", &properties2)
        .await
        .unwrap();
    ...
    let filtered_count = store
        .count_objects(object_type, Some(&[filter]))
        .await
        .unwrap();
    assert!(
        filtered_count >= 1,
        "Expected at least 1 filtered object, got {}",
        filtered_count
    );
...
async fn test_elasticsearch_bulk_index() {
    ...
    for i in 0..10 {
        ...
        properties.insert(
            "name".to_string(),
            PropertyValue::String(format!("Bulk Test {}", i)),
        );
    ...
    assert!(
        count >= 10,
        "Expected at least 10 objects after bulk index, got {}",
        count
    );
    ...
    for i in 0..10 {
        let _ = store
            .delete_object(object_type, &format!("bulk_{}", i))
            .await;
    }
}
...
async fn test_elasticsearch_alias_operations() {
    ...
    properties.insert(
        "name".to_string(),
        PropertyValue::String("Version 1".to_string()),
    );
    store
        .index_object(object_type, "v1_obj", &properties)
        .await
        .unwrap();
    ...
    properties2.insert(
        "name".to_string(),
        PropertyValue::String("Version 2".to_string()),
    );
    store
        .index_object(object_type, "v2_obj", &properties2)
        .await
        .unwrap();
    ...
    assert_eq!(
        version,
        Some(2),
        "Expected alias to point to version 2 after swap"
    );
...
async fn test_dgraph_traverse_with_filters() {
    ...
    // Create links
    let link1 = store
        .create_link(link_type, "source1", "target1", &link_props1)
        .await
        .unwrap();
    let link2 = store
        .create_link(link_type, "source1", "target2", &link_props2)
        .await
        .unwrap();
    ...
    let result = store
        .traverse_with_filters("source1", &[link_type.to_string()], 1, &[filter])
        .await;
    ...
    assert!(
        result.contains(&"target2".to_string()),
        "Expected target2 in filtered results"
    );
    assert!(
        !result.contains(&"target1".to_string()),
        "Expected target1 to be filtered out"
    );
...
async fn test_dgraph_traverse_with_aggregation() {
    ...
    store
        .create_link(link_type, "source_agg", "target_agg1", &link_props)
        .await
        .unwrap();
    ...
    let result = store
        .traverse_with_aggregation("source_agg", &[link_type.to_string()], 1, &aggregation)
        .await
        .unwrap();

    // Verify aggregation result structure
    // result.count is usize, always non-negative
...
async fn test_search_with_filters() {
    ...
    for i in 0..5 {
        ...
        properties.insert(
            "name".to_string(),
            PropertyValue::String(format!("Filter Test {}", i)),
        );
        properties.insert("score".to_string(), PropertyValue::Integer(i * 10));

        store
            .index_object(object_type, &format!("filter_{}", i), &properties)
            .await
            .unwrap();
    }
    ...
    assert!(
        results.len() >= 2,
        "Expected at least 2 filtered results, got {}",
        results.len()
    );
    ...
    for i in 0..5 {
        let _ = store
            .delete_object(object_type, &format!("filter_{}", i))
            .await;
    }
}
