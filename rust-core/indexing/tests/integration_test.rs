...
async fn test_complete_search_flow() {
    ...
    for i in 0..5 {
        ...
        properties.insert(
            "name".to_string(),
            PropertyValue::String(format!("Object {}", i)),
        );
        ...
        store
            .index_object(object_type, &format!("obj_{}", i), &properties)
            .await
            .unwrap();
    }
    ...
    assert!(
        total_count >= 5,
        "Expected at least 5 objects, got {}",
        total_count
    );
    ...
    for i in 0..5 {
        let _ = store
            .delete_object(object_type, &format!("obj_{}", i))
            .await;
    }
}
...
async fn test_bulk_indexing_flow() {
    ...
    for i in 0..20 {
        ...
        properties.insert(
            "name".to_string(),
            PropertyValue::String(format!("Bulk Object {}", i)),
        );
        properties.insert("index".to_string(), PropertyValue::Integer(i));
        properties.insert(
            "batch".to_string(),
            PropertyValue::String("batch1".to_string()),
        );
    ...
    assert!(
        count >= 20,
        "Expected at least 20 objects after bulk index, got {}",
        count
    );
...
async fn test_blue_green_migration_flow() {
    ...
    properties1.insert(
        "name".to_string(),
        PropertyValue::String("Version 1 Data".to_string()),
    );
    properties1.insert("version".to_string(), PropertyValue::Integer(1));

    store
        .index_object(object_type, "data1", &properties1)
        .await
        .unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    ...
    properties2.insert(
        "name".to_string(),
        PropertyValue::String("Version 2 Data".to_string()),
    );
    properties2.insert("version".to_string(), PropertyValue::Integer(2));

    store
        .index_object(object_type, "data2", &properties2)
        .await
        .unwrap();
    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    ...
    assert_eq!(
        version,
        Some(2),
        "Alias should point to version 2 after swap"
    );
...
async fn test_dgraph_traversal_flow() {
    ...
    let result = store
        .traverse("source_node", &[link_type.to_string()], 1)
        .await
        .unwrap();
    ...
    let filtered_result = store
        .traverse_with_filters("source_node", &[link_type.to_string()], 1, &[filter])
        .await
        .unwrap();

    // Should filter out nodes with weight <= 10
    assert!(
        filtered_result.len() < result.len(),
        "Filtered result should have fewer nodes"
    );
    ...
    let agg_result = store
        .traverse_with_aggregation("source_node", &[link_type.to_string()], 1, &aggregation)
        .await
        .unwrap();

    // Verify aggregation result structure
    // count is usize, always non-negative
